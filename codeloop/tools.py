...
class LocalTools:
    ...
    def list_files(self, path: str = ".", recursive: bool = True) -> ToolResult:
        try:
            ...
            if not root.exists():
                return ToolResult("list_files", False, error=f"path not found: {path}", error_type="file_not_found")
            if root.is_file():
            ...
            return ToolResult("list_files", False, error=str(exc), error_type="tool_error")

    def read_file(self, path: str) -> ToolResult:
        try:
            target = self.policy.resolve_workspace_path(path)
            if not target.is_file():
                return ToolResult("read_file", False, error=f"not a file: {path}", error_type="file_not_found")
            return ToolResult("read_file", True, output=target.read_text(encoding="utf-8"))
        except Exception as exc:
            return ToolResult("read_file", False, error=str(exc), error_type="tool_error")
    ...
    def write_file(self, path: str, content: str) -> ToolResult:
        try:
            ...
            return ToolResult("write_file", False, error=str(exc), error_type="tool_error")
    ...
    def search_code(self, pattern: str, path: str = ".", include: str = "*", max_matches: int = 50) -> ToolResult:
        try:
            ...
            return ToolResult("search_code", False, error=str(exc), error_type="tool_error")
    ...
    def apply_patch(self, patch: str) -> ToolResult:
        try:
            ...
            return ToolResult("apply_patch", False, error=str(exc), error_type="patch_format_error")
        except PatchContextError as exc:
            return ToolResult("apply_patch", False, error=str(exc), error_type="patch_context_error")
        except FileNotFoundError as exc:
            return ToolResult("apply_patch", False, error=str(exc), error_type="file_not_found")
        except Exception as exc:
            return ToolResult("apply_patch", False, error=str(exc), error_type="tool_error")
    ...
    def run_command(self, command: str, timeout: int | None = None) -> ToolResult:
        try:
            ...
            return ToolResult("run_command", False, error=str(exc), error_type="tool_error")
...
def apply_unified_patch(policy: RuntimePolicy, patch: str) -> list[str]:
    ...
    if not lines or lines[0] != "*** Begin Patch" or lines[-1] != "*** End Patch":
        raise PatchFormatError(
            "patch must use CodeLoop format exactly: *** Begin Patch, one or more "
            "*** Add File/Update File/Delete File sections, then *** End Patch"
        )
    ...
    while index < len(lines) - 1:
        ...
        if line.startswith("*** Add File: "):
            ...
            while index < len(lines) - 1 and not lines[index].startswith("*** "):
                if not lines[index].startswith("+"):
                    raise PatchFormatError("add file lines must start with +")
                new_lines.append(lines[index][1:])
                index += 1
            target = policy.resolve_workspace_path(rel)
            if target.exists():
                raise PatchContextError(f"file already exists: {rel}")
            target.parent.mkdir(parents=True, exist_ok=True)
            target.write_text("\n".join(new_lines) + "\n", encoding="utf-8")
            changed.append(rel)
            continue
            ...
            while index < len(lines) - 1 and lines[index].startswith("@@"):
                ...
                while index < len(lines) - 1 and not lines[index].startswith("@@") and not lines[index].startswith("*** "):
                    ...
                    if patch_line.startswith(" "):
                        ...
                        raise PatchFormatError(f"invalid patch line: {patch_line}")
                    index += 1
                position = _find_block(content_lines, old_block)
                if position < 0:
                    raise PatchContextError(f"hunk not found in {rel}")
        ...
        raise PatchFormatError(f"unknown patch operation: {line}")
